...
for n_clusters in range_n_clusters:
    ...
    print("Calinski Harabaz score: %.6f" % metrics.calinski_harabaz_score(X, cluster_labels))
    ...
    plt.suptitle(("                                                                                                                    KMeans clustering with wine dataset "
...
#             # 2nd Plot showing the actual clusters formed
#             colors = cm.Spectral(cluster_labels.astype(float) / n_clusters)
#             ax2.scatter( X_norm.iloc[:, q], X_norm.iloc[:, r], marker='.', s=30, lw=0, alpha=0.7,
#                         c=colors, edgecolor='k')
...
#             # Draw white circles at cluster centers
#             ax2.scatter(centers[:, q], centers[:, r], marker='o',
#                         c="white", alpha=1, s=200, edgecolor='k')

#             for i, c in enumerate(centers):
#                 ax2.scatter( c[q], c[r], marker='$%d$' % i, alpha=1,
#                             s=50, edgecolor='k')
...
#     # 2nd Plot showing the actual clusters formed
#     colors = cm.Spectral(cluster_labels.astype(float) / n_clusters)
#     plt.scatter( X_norm.iloc[:, q], X_norm.iloc[:, q-2], marker='.', s=30, lw=0, alpha=0.7,
#                 c=colors, edgecolor='k')
...
#     # Draw white circles at cluster centers
#     plt.scatter(centers[:, q], centers[:, q-2], marker='o',
#                 c="white", alpha=1, s=200, edgecolor='k')

#     for i, c in enumerate(centers):
#         ax.scatter( c[q], c[q-2], marker='$%d$' % i, alpha=1,
#                     s=50, edgecolor='k')
...
# pca = PCA(n_components=q, random_state=q-1)
# X_r = pca.fit(X).transform(X)
# X_pca = X_r
# print('explained variance ratio (first two components): %s'
#       % str(pca.explained_variance_ratio_))
